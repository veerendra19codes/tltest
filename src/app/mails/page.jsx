        ...
        <div className="flex flex-col gap-12 justify-center items-center">
            ...
            <div className="table w-4/5 h-full flex flex-col items-center justify-center gap-8">
                <div className="tablehead w-full flex flex-row mb-6" >
                    ...
                    <div className="w-1/5 py-2 pl-4 text-center font-bold">RejectedBy</div>

                </div>

                {data.map((d) => (
                    <div key={d.id} className="border-[1px] border-gray-500 rounded-xl w-full flex flex-row mb-8 py-4">
                        <div className="w-1/5 mx-2  pl-4  flex justify-center items-center">{d.companyname}</div>
                        <div className="w-1/5  mx-2 pl-4 flex justify-center items-center text-blue-500" ><a href={d.jobdetails} target="_blank" className="hover:underline">Clickhere</a></div>
                        {d.teamleader === "unassigned" ? (
                            ...
                            <div className="w-1/5 mx-2  pl-4 flex justify-center items-center">{d.teamleader}</div>

                        )}
                        <div className={d.status === "pending" ? " flex w-1/5  bg-red-600 text-white rounded-xl items-center justify-center h-auto mx-2" : "w-1/5  pl-4  items-center flex justify-center h-auto mx-2"}>{d.status}</div>
                        <div className="w-1/5 mx-2  pl-4 flex justify-center items-center">{d.rejectedTeamLeaders.length === 0 ? "none" : d.rejectedTeamLeaders}</div>
                    </div>
                ))}
            </div>
        </div >
