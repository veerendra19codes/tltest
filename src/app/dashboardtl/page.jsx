...
const DashboardTLPage = async () => {
    ...
    const data = users.filter((data) => data.teamleader === session.user?.id);
            ...
            <div className="table w-4/5 h-full mt-12 flex flex-col items-center justify-center gap-8">
                ...
                {data.map((d) => (
                    <div key={d.id} className="border-[1px] border-gray-500 rounded-xl w-full flex flex-row mb-8 py-4">
                        ...
                        <div className={d.companiesWorking === "none" ? "w-1/5   items-center flex justify-center h-auto mx-2 bg-green-500 rounded-xl text-white" : "w-1/5  items-center flex justify-center h-auto mx-2"}>{d.companiesWorking}</div>
                        <div className="w-1/5   items-center flex justify-center h-auto mx-2">{d.companiesCompleted}</div>
                        <div className="w-1/5  items-center flex justify-center h-auto mx-2">{d.companiesRejected}</div>
                    </div>
                ))}
            </div>
        </div >
